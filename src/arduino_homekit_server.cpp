...
#pragma region pair_verify_context_new
...
#pragma region pair_verify_context_free
...
#pragma region client_context_new
//==========================
...
#pragma region client_context_free
void client_context_free(client_context_t* c)
{
  if(c->verify_context)
...
#pragma region pairing_context_free
void pairing_context_free(pairing_context_t* context)
{
  if(context == saved_preinit_pairing_context)
  {
    VERBOSE("Free saved_preinit_pairing_context");
...
#pragma region client_send_encrypted_
...
#pragma region client_decrypt_
int client_decrypt_(client_context_t* context, byte* payload, size_t payload_size, byte* decrypted, size_t* decrypted_size)
{
  if(!context || !context->encrypted)
    return -1;
...
#pragma region client_notify_characteristic
void client_notify_characteristic(homekit_characteristic_t* ch, homekit_value_t value, void* context)
{
  ...
  if(client->current_characteristic == ch
    ...
    && homekit_value_equal(client->current_value, &value))
  {
    ...
    CLIENT_VERBOSE(client, "This value is set by this client, no need to send notification");
    return;
  }
  ...
  CLIENT_INFO(client, "Sending event %s", ch->description);
...
#pragma region client_send_chunk
...
#pragma region send_client_events
...
#pragma region send_tlv_response
void send_tlv_response(client_context_t* context, tlv_values_t* values)
{
...
#pragma region send_json_error_response
...
#pragma region homekit_get_client_id
...
#pragma region homekit_client_is_admin
...
#pragma region homekit_client_send
...
#pragma region homekit_server_on_identify
void homekit_server_on_identify(client_context_t* context)
{
  CLIENT_INFO(context, "On Identify"); DEBUG_HEAP();
...
#pragma region homekit_server_on_pair_setup
    ...
    case 5:
      {
        ...
        VERBOSE("Added pairing with %s", device_id);
        free(device_id);
...
#pragma region homekit_server_on_pair_verify
...
void homekit_server_on_get_accessories(client_context_t* context)
{
  ...
  for(homekit_accessory_t** accessory_it = const_cast<homekit_accessory_t**>(context->server->config->accessories);
    *accessory_it;
    ++accessory_it)
  {
    ...
    for(homekit_service_t** service_it = const_cast<homekit_service_t**>(accessory->services);
      *service_it;
      ++service_it)
    {
      ...
      for(homekit_characteristic_t** ch_it = const_cast<homekit_characteristic_t**>(service->characteristics); *ch_it; ch_it++)
      {
...
#pragma region bool_endpoint_param
...
#pragma region write_characteristic_error
...
#pragma region process_characteristics_update
HAPStatus process_characteristics_update(const cJSON* j_ch, client_context_t* context)
{
  ...
  if(j_value)
  {
    ...
    /* 2024-09-12 Holger Burkarth:
     * If the characteristic has a setter_ex function, we call it directly
     * instead of updating the value directly. This is to allow the setter
     * to perform additional validation or processing.
     *
     * Temporary zeroing of the function pointer prevents recurrence.
    */

    typedef void (*setter_ex)(homekit_characteristic_t* ch, const homekit_value_t value);
    setter_ex setter_func = ch->setter_ex;

    #define INVOKE_SETTER(ch, h_value) \
      ch->setter_ex = nullptr; \
      setter_func(ch, h_value); \
      ch->setter_ex = setter_func;
...
client_context_t* homekit_server_accept_client(homekit_server_t* server)
{
  ...
  if(server->wifi_server->hasClient())
  {
    ...
    {
      WARN("No more room for client connections (max %d)", HOMEKIT_MAX_CLIENTS);
      wifiClient->stop();
      delete wifiClient;
      return NULL;
    }
...
void homekit_server_process_notifications(homekit_server_t* server)
{
  ...
  // haracteristic_event_t client_event_t
  // Apple，Notify
  while(context)
  {
...
void homekit_mdns_init(homekit_server_t* server)
{
  ...
  homekit_accessory_t* accessory = const_cast<homekit_accessory_t*>(server->config->accessories[0]);
...
void homekit_server_init(const homekit_server_config_t* config)
{
  ...
  if(config->config_number == 0)
  {
    ERROR("Error initializing HomeKit accessory server: config_number is not specified");
  ...
  if(!config->category)
  {
    ERROR("Error initializing HomeKit accessory server: category is not specified");
...
bool arduino_homekit_preinit(homekit_server_t* server)
{
  ...
  INFO("Preinitializing pairing context");
  ...
  INFO("Preinitialize pairing context success");
...
void arduino_homekit_setup(const homekit_server_config_t* config)
{
  if(system_get_cpu_freq() != SYS_CPU_160MHZ)
  {
    ...
    VERBOSE("Update the CPU to run at 160MHz");
  }
