...
$AUT Holger Burkarth
$DAT >>types.h<< 20 Sep 2024  06:57:26 - (c) proDAD
*******************************************************************/
#pragma endregion
...
extern "C" {
  ...
  #pragma region homekit_format_t
  typedef enum
  {
    homekit_format_bool,
  ...
  #pragma region homekit_permissions_t
  typedef enum
  {
  ...
  #pragma region homekit_accessory_category_t
  typedef enum
  {
  ...
  #define HOMEKIT_CSTRING_(value, ...) \
    {.format=homekit_format_string, .cstring_value=(value), ##__VA_ARGS__}
  #define HOMEKIT_CSTRING(value, ...) (homekit_value_t) HOMEKIT_CSTRING_(value, ##__VA_ARGS__)
  ...
  #pragma region homekit_valid_values_t
  typedef struct
  {
    int count;
  ...
  #pragma region homekit_valid_values_range_t
  typedef struct
  {
    uint8_t start;
    uint8_t end;
  ...
  #pragma region homekit_valid_values_ranges_t
  typedef struct
  {
    int count;
    const homekit_valid_values_range_t* ranges;
  ...
  #pragma region homekit_characteristic_change_callback_t
  ...
  #pragma region _homekit_characteristic
  struct _homekit_characteristic
  {
    ...
    homekit_unit_t unit;
    /*homekit_permissions_t*/ uint8_t permissions;
    ...
    void* context;
    void* UserPtr; // Free to use by user
  };

  #pragma endregion

  #pragma region _homekit_service
  struct _homekit_service
  {
    ...
    const homekit_characteristic_t** characteristics;
  };

  #pragma endregion

  #pragma region _homekit_accessory
  struct _homekit_accessory
  {
    unsigned int id;
    ...
    const homekit_service_t** services;
  };

  #pragma endregion

  #pragma region HOMEKIT_ACCESSORY
  ...
  #pragma region HOMEKIT_CHARACTERISTIC
  ...
  #pragma region HOMEKIT_DECLARE_CHARACTERISTIC_CUSTOM
  // Declaration macro to create a custom characteristic in-place without
...
// either allocated on heap or in static memory (but not on stack).
  ...
  #pragma region NEW_HOMEKIT_ACCESSORY
  ...
  #pragma region NEW_HOMEKIT_SERVICE
  ...
  #pragma region NEW_HOMEKIT_CHARACTERISTIC
  ...
  /* Convert homekit_accessory_category_t to string.
   * @param cat: homekit_accessory_category_t value.
   * Returns "unknown" if category is unknown.
  */
  const char* homekit_accessory_category_to_string(homekit_accessory_category_t cat);

  /* Convert homekit_unit_t to string.
   * @param type like HOMEKIT_CHARACTERISTIC_ON @see homekit_characteristic_t::type
   * Returns "unknown" if unit is unknown.
  */
  const char* homekit_characteristic_identify_to_string(const char* type);

  /* Convert homekit_unit_t to string.
   * @param type like HOMEKIT_SERVICE_SWITCH @see homekit_service_t::type
   * Returns "unknown" if unit is unknown.
  */
